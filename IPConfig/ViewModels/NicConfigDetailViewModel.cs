    ...
    IRecipient<PropertyChangedMessage<Nic>>
{
    ...
    [ObservableProperty]
    private string _interfacePriorityNote = "Applies to all physical Wi-Fi and Ethernet adapters. Lower metric = higher priority.";

    [ObservableProperty]
    private string _ipv4PrioritySummary = "Loading...";

    [ObservableProperty]
    private string _ipv6PrioritySummary = "Loading...";
    ...
    [RelayCommand]
    private async Task PreferEthernetAsync()
    {
        await ApplyConnectionPriorityPresetAsync(ConnectionType.Ethernet,
            "Ethernet is now preferred for Internet routing.");
    }

    [RelayCommand]
    private async Task PreferWiFiAsync()
    {
        await ApplyConnectionPriorityPresetAsync(ConnectionType.Wlan,
            "Wi-Fi is now preferred for Internet routing.");
    }

    [RelayCommand]
    private async Task ResetConnectionPriorityAsync()
    {
        try
        {
            await Task.Run(NetworkManagement.ResetConnectionPriority);
            RefreshInterfacePriority();
            Messenger.Send<RefreshMessage>(new(this));

            Growl.Success(new() {
                Message = "Interface priority was reset to automatic metric.",
                WaitTime = 2
            });
        }
        catch (Exception ex)
        {
            Growl.Error($"Failed to reset interface priority.\n\n{ex.Message}");
        }
    }
    ...
    private async Task ApplyConnectionPriorityPresetAsync(ConnectionType preferredType, string successMessage)
    {
        try
        {
            await Task.Run(() => NetworkManagement.PreferConnectionType(preferredType));
            RefreshInterfacePriority();
            Messenger.Send<RefreshMessage>(new(this));
        ...
        {
            Growl.Error($"Failed to update interface priority.\n\n{ex.Message}");
        }
    }

    private void RefreshInterfacePriority()
    {
        if (Nic is null)
        {
            IPv4PrioritySummary = "Not available";
            IPv6PrioritySummary = "Not available";
            return;
        }

        var ipv4Info = Nic.IPv4InterfaceProperties is null
            ? new InterfaceMetricInfo(null, null)
            : NetworkManagement.GetInterfaceMetricInfo(Nic.IPv4InterfaceProperties.Index, System.Net.Sockets.AddressFamily.InterNetwork);

        var ipv6Info = Nic.IPv6InterfaceProperties is null
            ? new InterfaceMetricInfo(null, null)
            : NetworkManagement.GetInterfaceMetricInfo(Nic.IPv6InterfaceProperties.Index, System.Net.Sockets.AddressFamily.InterNetworkV6);

        IPv4PrioritySummary = ipv4Info.ToDisplayString();
        IPv6PrioritySummary = ipv6Info.ToDisplayString();
    }
